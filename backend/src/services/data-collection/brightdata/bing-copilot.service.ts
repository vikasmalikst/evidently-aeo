/**
 * Bing Copilot collector service via BrightData
 */
...
export class BrightDataBingCopilotService extends BaseBrightDataService {
  ...
  async executeQuery(request: BrightDataRequest): Promise<BrightDataResponse> {
    return await this.executeBingCopilotAsync(request);
  }

  /**
   * Execute Bing Copilot query asynchronously using trigger endpoint
   */
  private async executeBingCopilotAsync(request: BrightDataRequest): Promise<BrightDataResponse> {
    const collectorType = 'bing_copilot';
    const datasetId = this.getDatasetId(collectorType);
    this.validateConfig(collectorType);

    try {
      console.log(`🚀 Executing Bing Copilot query via BrightData Async (dataset: ${datasetId})`);
      ...
      console.log(`📡 Triggering async Bing Copilot request: ${triggerUrl}`);
      ...
      console.log(`📡 Bing Copilot trigger response status: ${response.status}`);

      if (!response.ok) {
        const errorText = await response.text();
        console.error('❌ BrightData Bing Copilot trigger error:', {
          status: response.status,
          statusText: response.statusText,
          body: errorText
        });
        throw new Error(`BrightData Bing Copilot API error: ${response.status} ${response.statusText} - ${errorText}`);
      }

      const result = await response.json() as any;
      console.log(`✅ Bing Copilot trigger response:`, JSON.stringify(result, null, 2));
      ...
      if (!snapshotId) {
        ...
        throw new Error('BrightData Bing Copilot trigger did not return snapshot_id');
      }
      ...
      console.error('❌ BrightData Bing Copilot async error:', error.message);
      throw error;
    }
  }
