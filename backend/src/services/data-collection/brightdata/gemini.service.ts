/**
 * Gemini collector service via BrightData
 */
...
export class BrightDataGeminiService extends BaseBrightDataService {
  ...
  async executeQuery(request: BrightDataRequest): Promise<BrightDataResponse> {
    return await this.executeGeminiAsync(request);
  }

  /**
   * Execute Gemini query asynchronously using trigger endpoint
   */
  private async executeGeminiAsync(request: BrightDataRequest): Promise<BrightDataResponse> {
    const collectorType = 'gemini';
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`BrightData Gemini API error: ${response.status} ${response.statusText} - ${errorText}`);
      }
      ...
      console.error('❌ BrightData Gemini async error:', error.message);
      throw error;
    }
  }
