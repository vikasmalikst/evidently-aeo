/**
 * Google AIO collector service via BrightData
 */
...
export class BrightDataGoogleAIOService extends BaseBrightDataService {
  ...
  async executeQuery(request: BrightDataRequest): Promise<BrightDataResponse> {
    return await this.executeGoogleAIOAsync(request);
  }

  /**
   * Execute Google AIO query asynchronously using trigger endpoint
   */
  private async executeGoogleAIOAsync(request: BrightDataRequest): Promise<BrightDataResponse> {
    const collectorType = 'google_aio';
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`BrightData Google AIO API error: ${response.status} ${response.statusText} - ${errorText}`);
      }
      ...
      return {
        query_id: `brightdata_google_aio_${Date.now()}`,
        ...
        metadata: {
          provider: 'brightdata_google_aio',
      ...
      console.error('❌ BrightData Google AIO async error:', error.message);
      throw error;
    }
  }
