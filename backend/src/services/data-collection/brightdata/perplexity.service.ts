...
export class BrightDataPerplexityService extends BaseBrightDataService {
  ...
  private async executePerplexityAsync(request: BrightDataRequest): Promise<BrightDataResponse> {
    ...
    try {
      console.log(`🚀 Executing Perplexity query via BrightData Async (dataset: ${datasetId})`);
      ...
      console.log(`📡 Triggering async Perplexity request: ${triggerUrl}`);
      ...
      console.log(`📡 Perplexity trigger response status: ${response.status}`);
      ...
      console.log(`✅ Perplexity trigger response:`, JSON.stringify(result, null, 2));
